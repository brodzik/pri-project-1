...
\node [rect, below of=start] (input) {wejście linii znaków};
\node [rect, below of=input] (iter) {iteracja po każdym znaku};
\node [rect, below of=input] (iter) {zastąpienie znaków};
\node [rect, below of=start] (stop) {stop};
\path [line] (start) -- (input);
\path [line] (input) -- (iter);
\end{tikzpicture}
\end{center}
